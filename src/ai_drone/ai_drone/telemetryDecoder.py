...
_ORIGIN_ALT = float(os.getenv("TELEM_ORIGIN_ALT", "35.0"))  # meters
_YAW_OFFSET = float(os.getenv("TELEM_YAW_OFFSET", "0.0"))    # degrees
...
def _latlon_to_enu_m(lat: float, lon: float, alt_m: float):
    ...
    # See: Defense Mapping Agency formulas (approx)
...
def normalize_incoming(js: Dict[str, Any]) -> Dict[str, Any]:
    ...
    alt  = float(js.get("alt_m", js.get("alt", 0.0)))  # accept alt alias
